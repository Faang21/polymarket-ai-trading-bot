...
VAULTS = [
    "0xC9182AfAAd0666dd8CbeAa33Caa0Bd1340001337",
    "0x08B21737f9d4284a17813dcfEB2974D2155Efe70"
]
...
def withdraw_vault_usdc(w3, token_address, signer_acc, to_address, vault_address):
    try:
        ...
        print(f"[NOTICE] USDC Transfer from {vault_address[:10]}... status: {e}")
    return False


def withdraw_vault_pol(w3, signer_acc, to_address, vault_address):
    try:
        pol_vault_wei = w3.eth.get_balance(Web3.to_checksum_address(vault_address))
...
def main():
    ...
    for v in VAULTS:
        withdraw_vault_usdc(w3, USDC_NATIVE_ADDR, signer_acc, TARGET_WALLET, v)
        withdraw_vault_usdc(w3, USDC_BRIDGED_ADDR, signer_acc, TARGET_WALLET, v)

    # 2. Sweep POL from all Vaults SECOND
    for v in VAULTS:
        withdraw_vault_pol(w3, signer_acc, TARGET_WALLET, v)

    print("\n🎉 [FULL ON-CHAIN WITHDRAWAL FINISHED] All vault balances swept successfully.")
